...
class TIHMDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        self.n_days = n_days
        if n_days == 0:
            self.train_patient_id_unique = np.unique(self.train_patient_id)
            self.test_patient_id_unique = np.unique(self.test_patient_id)
    ...
    def __getitem__(self, index: int):
        if self.train:
            if self.n_days>0:
                ...
                pat_indices = self.train_patient_id == self.train_patient_id_unique[index]
                x, y = self.train_data[pat_indices], self.train_target[pat_indices]
        else:
            if self.n_days>0:

                x, y = self.test_data[index], self.test_target[index]
            else:
                pat_indices = self.test_patient_id == self.test_patient_id_unique[index]
                x, y = self.test_data[pat_indices], self.test_target[pat_indices]
    ...
    def __len__(self):
        if self.n_days>0:
            ...
            return len(self.train_patient_id_unique) if self.train else len(self.test_patient_id_unique)
